...
#pragma omp parallel for private (j)
    ...
    for(r = 0; r < repeats; r++) {
#pragma omp parallel for private(acc, k, j)
        for(int ii = 0; ii < N; ii+=tile_size) {
            for(j = 0; j < N; j += 2) {
                for(i = ii; i < ii+tile_size; i += 2) {
                  acc = 0;
                  double acc01 = 0;
                  double acc10 = 0;
                  double acc11 = 0;
                  for(k = 0; k < N; k++) {
                    acc += matrixA[i*N + k] * matrixB[k*N + j];
                    acc01 += matrixA[(i+1)*N + k] * matrixB[k*N + j];
                    acc10 += matrixA[i*N + k] * matrixB[k*N + (j+1)];
                    acc11 += matrixA[(i+1)*N + k] * matrixB[k*N + (j+1)];
                  }
                  matrixC[i*N + j] = (alpha * acc) + (beta * matrixC[i*N + j]);
                  matrixC[(i+1)*N + j] = (alpha * acc) + (beta * matrixC[(i+1)*N + j]);
                  matrixC[i*N + (j+1)] = (alpha * acc) + (beta * matrixC[i*N + (j+1)]);
                  matrixC[(i+1)*N + (j+1)] = (alpha * acc) + (beta * matrixC[(i+1)*N + (j+1)]);
                }
            }
        }
    }
